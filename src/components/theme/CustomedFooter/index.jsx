...
const SocialFoterIcon = ({ label, ...props }) => {
    switch (label) {
        case "facebook":
            return (
...
const InfoName = ({ label, ...props }) => {
    switch (label) {
        case "coppyright":
            return (
                <span>Coppyright 2021</span>
            )
...
export const CustomedFooter = () => {
    ...
    const { wpMain, text } = useStaticQuery(graphql`
    {
      wpMain(slug: {eq: "social-menu"}) {
        social_menu_footer {
          facebook
          instagram
          phone
  ...
  `)
    ...
    if (!wpMain?.social_menu_footer || wpMain.social_menu_footer === 0) return null

    const socials = wpMain.social_menu_footer

    const info = wpMain.footer_info
